package com.android.base.utils.dialog;
...
import com.android.base.utils.ViewUtil;
...
public class CustomDialog extends Dialog {
     ...
     * <br> UpdateInfo: (此处输入修改内容, 若无修改可不写.)
     */
    public void showIosDialog(String title, final ArrayList items, int gravity, int[] layout_margin, HashMap<Integer, Integer> positionColor, final OnDialogClickListener listener) {
     ...
     *         dp
     */
    public void setDialogWidth(int gravity, int dp) {
        if (null != window) {
            window.setGravity(gravity);
            WindowManager.LayoutParams lp = window.getAttributes();
            lp.width = ScreenUtil.getScreenWidthPx() - ScreenUtil.dip2px(dp); // 设置宽度
            window.setAttributes(lp);
            window.setWindowAnimations(R.style.popupwindow_view_anim);
        }
    }
